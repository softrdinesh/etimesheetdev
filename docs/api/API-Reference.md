...
| `isProjectTask` | bool? | no | — | `true` when `taskId` is a project task, `false` when it is a sprint task — the two lists [Task List](#4-task-list) returns, whose ids can coincide. Stored as sent, `null` included |
...
14. **`isProjectTask` is optional on Save Time Log.** A missing value is stored
    as `null`, and `null` does not say which list `taskId` came from.
